        ...
        for (complist, is_cross) in ctypes:
            for langname, compiler in complist.items():
                ...
                if mesonlib.is_windows():
                    command_template = ''' command = {executable} @$out.rsp
 rspfile = $out.rsp
 rspfile_content = $ARGS  {output_args} -Wl,--start-group $in $LINK_ARGS {cross_args} $aliasing
'''
                else:
                    command_template = ' command = {executable} $ARGS {output_args} -Wl,--start-group $in $LINK_ARGS {cross_args} $aliasing\n'
                command = command_template.format(
